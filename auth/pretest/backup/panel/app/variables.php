...
<?php
    ...
    if (isset($_POST['variable_create']))
    {
        ...
        else
        {
            ...
            {
                $var_verify = mysqli_query($con, "SELECT * FROM `vars` WHERE `owner` = '$username' AND `name` = '$variable_name'") or die(mysqli_error($con));
                ...
                else
                {
                    ...
                    if (mysqli_num_rows($result) > 0)
                    {
                        ...
                        $sending_collection = mysqli_query($con, "INSERT INTO `vars` (name, value, programtoken, owner) VALUES ('$variable_name', '$variable_value', '$app_key', '$username')") or die(mysqli_error($con));
    ...
    if (isset($_POST['variable_delete']))
    {
        ...
        else
        {
            ...
            if (mysqli_num_rows($result) > 0)
            {
                ...
                $var_delete = mysqli_query($con, "DELETE FROM `vars` WHERE `owner` = '$username' AND `programtoken` = '$app_key' AND `name` = '$variable_name'") or die(mysqli_error($con));
                ...
                <div class="row layout-top-spacing">
                                        ...
                                        if (isset($_POST['var_key_create']))
                    {
                        ...
                        if (mysqli_num_rows($result) > 0)
                        {
                            while ($row = mysqli_fetch_array($result))
                            {
                                $application_key = $row['authtoken'];
                            }

                            $update_var = mysqli_query($con, "UPDATE `programs` SET `variablekey` = '$key' WHERE `owner` = '$username' AND `id` = '$id' AND `authtoken` = '$application_key'") or die(mysqli_error($con));
                            if ($update_var)
                            {
                                ...
                                echo '
                                    ...
                                    notyf
                                    .success({
                                        message: \'Successfully generated a new variable secret key!\',
                                        duration: 3500,
                                        dismissible: true
                                    });                
                            ...
                            else
                            {
                                ...
                                echo '
                                    ...
                                    notyf
                                    .error({
                                        message: \'Failed to generate variable secret key!\',
                                        duration: 3500,
                                        dismissible: true
                                    });                
                    ...
                    //  $owner = xss_clean(mysqli_real_escape_string($con, $_SESSION['username']));
                     ...
                     $var_query = mysqli_query($con, "SELECT * FROM `programs` WHERE `owner` = '$username' AND `id` = '$id'") or die(mysqli_error($con));
                    ...
                    <input type="text" class="form-control mb-4" value="<?php echo $var_key ?>" disabled>

                    <form method="POST" action="">
                        <button class="btn btn-primary mb-2" name="var_key_create">Generate new variable key</button>
                    </form>
