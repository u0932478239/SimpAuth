	...
	if(isset($_GET['action'])) {
		...
		switch ($action) {
			...
			case 'totalusers':
			    ...
			    case "deletelicense":
    				header('Content-Type: application/json');
    					if(isset($_GET['program_key']) && isset($_GET['api_key']) && isset($_GET['license'])) {
    				    
    				    
    					$api_key = $_GET['api_key'];
    					$authtoken = $_GET['program_key'];
    					$license_to = $_GET['license'];
    					...
    					if (mysqli_num_rows($api_exist) > 0)
    					{
    					    ...
    					    if (mysqli_num_rows($program_exist) > 0)
    					    {
    					        ...
    					        {
            			            $verify_license = mysqli_query($con, "SELECT * FROM `tokens` WHERE `token` = '$license_to' AND `programtoken` = '$authtoken'") or die(mysqli_error($con));
            			            if (mysqli_num_rows($verify_license) > 0)
            			            {
            			                $del_license = mysqli_query($con, "DELETE FROM `tokens` WHERE `token` = '$license_to' AND `programtoken` = '$authtoken'") or die(mysqli_error($con));
            			                if ($del_license)
            			                {
            			            ...
            			            {
            			                echo json_encode('License not found!');
            			            }
    					        }
    					...
    					if (mysqli_num_rows($api_exist) > 0)
    					{
    					    ...
    					    if (mysqli_num_rows($program_exist) > 0)
    					    {
    					        ...
    					        else
    					        {
            			            ...
            			            if (mysqli_num_rows($verify_license) > 0)
            			            {
            			                while ($row_lice = mysqli_fetch_array($verify_license))
            			                {
            			                    $used = ($row_lice['used'] == '1') ? 'Yes' : 'No';
            			                    $usedby = $row_lice['used_by'];
            			                    $expiry = $row_lice['days'];
            			                    $level = $row_lice['level'];
            			                }
